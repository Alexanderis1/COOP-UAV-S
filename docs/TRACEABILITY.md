...
| PHY-UAV-010 (FCU + mission computer split) | Legacy mode: single agent node per UAV (`InterceptorUav`). Sitl mode (P4-3): real split — CoopFC FCU per vehicle + `mc/interceptor_app.py` on its own `sil/host.py` VirtualMCU (own clock, crash fence), talking over the byte-honest `coop_link` channel pair; tactical stack flies EKF estimates only | representative (sitl) / placeholder (legacy) | MC processor-crash fault mode free via the exception fence (SIM-SIL-003); fire interlock shared single-source (`mc/fire_control.py`), byte-equivalence pinned by sitl twins. |
| PHY-UAV-011 (nav sensors, GNSS-denied) | UAV truth state used directly for own-ship navigation (live path); staged P3 `coopfc/estimation/` error-state EKF over the staged P2 device suite closes the nav-error-model gap (wired in P4) | partial (staged) | Live path still truth-state until P4. Staged EKF validated by NEES/NIS MC + GNSS-denied drift suite (`tests/test_coopfc_ekf_mc.py` @slow): 5-min denial drift is km-class free-inertial with honest 4-sigma covariance claim (RESEARCH.md "P3 CoopFC flight stack"); the VIO/datalink fallback that makes 5-min denial *engagement-grade* is real-system scope, NOT simulated — requirement stays partial by design. |
...
| PHY-UAV-013 (health telemetry ≥ 1 Hz) | `UavState` (battery, ammo, mode, link) at node rate + P5-4 `health` digest: CBIT fault word/codes, inhibit flags, degraded mode — FCU HEALTH wire merged with MC-side faults; per-cell BMS taps + real SOC on the wire (P5-1f); recorded in frames (ICD v0.5) | high | Temperature channel absent (no thermal model). |
...
| PHY-UAV-021 (no release without clearance) | Shooter FSM requires AUTHORIZED `FireClearance`; turrets identically interlocked; orchestrator/posture gate northbound. P5-5 (sitl): release is HARD-interlocked in the FCU — clearance token mirrored over `coop_link`, WEAPON_RELEASE refused unless ARMED + CBIT-clean + token matches the track inside the shared freshness window; the round leaves the rail only on the FCU's effector pulse (NACK-by-timeout restores it) | high | Cryptographic token signing abstracted to message identity. |
...
| PHY-UAV-033 (link-loss autonomy, no self-authorisation) | Comms model drops/delays clearances; shot discipline holds/aborts; RTB behaviours. P5: CBIT degraded modes per fault (FAILSAFE_ATT/LAND/RTL, pinned priority chain), LINK_C2_LOSS inhibits release even with a token in hand, interlock pinned under EVERY injected fault kind (test_sitl_degraded matrix), MC-crash/link-jam starve the FCU home | high | Pre-authorised-constraint continuation policy minimal (current task continues; no timeout-RTB parameter yet). |

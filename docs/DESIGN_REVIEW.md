...
| # | Severity | Finding |
|---|----------|---------|
| 5.1 | HIGH **RESOLVED** (P0-6/P0-7, 2026-06-11) | ~~Single shared `world.rng` consumed in call order~~ — fixed by `core/rng.py RngRegistry`: per-consumer name-keyed streams (`weather`, `comms`, `sensor/*`, `adjudicator`, `debris`, `threat/*`); order-independence proven (`tests/test_rng_streams.py`); 10-seed re-baseline in `docs/reports/rng_rebaseline.md`. Process-global track-id/msg-seq counters remain (reset per World; see ORDERING.md §4). Original finding: **Single shared `world.rng` consumed in call order**: every sensor/adjudication/debris/comms draw depends on exact callback sequencing. ROS 2 executors do not guarantee that order → same seed, different battle. No per-node RNG streams. Together with process-global track-id/msg-seq resets, the *determinism contract* does not survive the migration as designed — "replaces two small classes" is understated. |
| 5.2 | MED **DOC'D** (P0-9, 2026-06-11: contract now explicit in `docs/ORDERING.md`, pinned by `tests/test_ordering.py`; semantics gap itself remains until the port) | **Synchronous-bus semantics ≠ ROS 2** (`core/bus.py`, `core/node.py`): immediate in-stack delivery, ordering inherited from scenario construction order, no cycle guard, missed ticks rebased — behaviours the test suite currently depends on. |
| 5.3 | MED **RESOLVED** (P0-8, 2026-06-11) | ~~Display rate inside the control loop~~ — `DebrisReporter` now has its own `debris_hz` scenario knob (default 5.0); `record_hz` no longer steers C2 tasking timing. Test in `tests/test_debris_live.py`. |
